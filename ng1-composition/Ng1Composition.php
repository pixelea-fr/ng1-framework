...
class ng1Composition {
    public function __construct() {
        ...
        add_action('save_post',  array($this,'create_block_pattern_from_pattern'));
    ...
    function create_block_pattern_from_pattern($post_id) {
    
        if ('pattern' === get_post_type($post_id)) {

            // Récupérer les données du post de type "pattern"
            ...
            // Créer le nom du bloc de modèle
            $block_pattern_name = 'ng1-pattern/' . get_post_field('post_name', $post_id);
    
            // Vérifier si le bloc de modèle existe déjà, s'il n'existe pas, l'enregistrer
            if (!wp_unslash(get_theme_mod('core-block-patterns', array())) || !in_array($block_pattern_name, wp_unslash(get_theme_mod('core-block-patterns', array())))) {
                $all_block_patterns = (array) get_theme_mod('core-block-patterns', array());
                $all_block_patterns[] = $block_pattern_name;
    
                set_theme_mod('core-block-patterns', $all_block_patterns);
            }
    
            // Enregistrez le bloc de modèle avec les données pertinentes
            register_block_pattern(
                $block_pattern_name,
                array(
                    'title' => $title,
    ...
    // Attacher le hook à l'action save_post
